...
begin
  ...
  begin
    ...
    for Contador := 0 to TActionList(FActions).ActionCount - 1 do
      // Se o caption estiver vazio, o hint aparecerá no lugar do caption na janela de permissões
      // Ridoval Cavalcanti - 30/07/2017
      if TAction(TActionList(FActions).Actions[Contador]).Caption = '' then
        FTempLista.Append(TActionList(FActions).Actions[Contador].Category + #1 +
          TActionList(FActions).Actions[Contador].Name + #2 +
          TAction(TActionList(FActions).Actions[Contador]).Hint)
      else
        FTempLista.Append(TActionList(FActions).Actions[Contador].Category + #1 +
          TActionList(FActions).Actions[Contador].Name + #2 +
